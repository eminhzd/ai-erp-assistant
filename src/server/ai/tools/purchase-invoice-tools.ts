...
export function createPurchaseInvoiceTools(companyId: number) {
  return {
    ...
    createPurchaseInvoice: tool({
      description:
        'Create a purchase invoice for the current company. Use this tool only when the user explicitly asks to create a purchase invoice. The supplier ID, warehouse ID, and at least one invoice item are required. Each item requires a product ID, quantity, and unit price. Invoice number and currency are generated automatically by the system and must not be provided by the model. Discount, tax, due date, issue date, and notes are optional. Do not invent suppliers, warehouses, products, quantities, prices, or other missing information. Creating the invoice also creates its purchase stock movements, so do not separately increase warehouse stock for the same purchase.',
      inputSchema: z.object({
